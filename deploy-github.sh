...
# Všechny operace musí probíhat ve složce, ve které leží tento skript.
PROJECT_DIR=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)

if [ "$PROJECT_DIR" = "/" ]; then
    echo "Chyba: Skript nelze spustit z kořenové složky systému!"
    exit 1
fi

cd "$PROJECT_DIR"
echo "Pracovní složka: $PROJECT_DIR"
